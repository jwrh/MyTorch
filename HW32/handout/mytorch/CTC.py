...
class CTC(object):
    ...
    def get_backward_probs(self, logits, extended_symbols, skip_connect):
        ...
        # -------------------------------------------->
        betahat[T-1,S-1] =  logits[T-1,S-1]
        betahat[T-1,0:S-2] = 0
        for t in range(T-2,-1,-1):
            betahat[t,S-1] = logits[t,extended_symbols[S-1]]*betahat[t+1,S-1]
            for i in range(S-2,-1,-1):
                betahat[t,i] = logits[t,extended_symbols[S-1]]*(betahat[t+1,i]+betahat[t+1,i+1])
        for u in range(T-1,-1,-1):
            for k in range(S-1,-1,-1):
                beta[u,k] = betahat[u,k]/logits[u,k]        
        # <--------------------------------------------
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            # -------------------------------------------->
            self.target[batch_itr] = self.target[0:target_lengths[batch_itr]] 
            self.logits[:,batch_itr] = self.logits[0:input_lengths[batch_itr]]
            (self.extended_symbols,self.skip_connect) = self.ctc.extend_target_with_blank(self.target)
            self.alpha = self.ctc.get_forward_probs(self.logits, self.extended_symbols, self.skip_connect)
            self.beta = self.ctc.get_backward_probs(self.logits, self.extended_symbols, self.skip_connect)
            self.gammas = self.ctc.get_posterior_probs(self.alpha,self.beta)
            total_loss[batch_itr] = (self.target[i]-self.gammas[i] for i in len(self.gammas))
