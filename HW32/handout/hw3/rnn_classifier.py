    ...
    def forward(self, x, h_0=None):
        ...
        for k in range(seq_len):
            ...
            for i in range(length):
                h = (lambda x,y : self.rnn[i].forward(x,y))(input, self.hiddens[-1][i])
                input = h
                hidden.append(h)
            groundTruth = copy.deepcopy(hidden)
            self.hiddens.append(groundTruth)

        # Get the outputs from the last time step using the linear layer and return it
        output = (lambda x: self.output_layer.forward(x))(input)
        return output
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for k in range(seq_len-1, -1, -1):
            for i in range(length, -1, -1):
                if i!= 0:
                    h_prev_l = self.hiddens[k+1][i-1] 
                else:
                    h_prev_l = self.x[:, k, :]
                h_prev_t = self.hiddens[k][i]
                dx, new_dh = ((lambda x,y,z,u: self.rnn[i].backward(x,y,z,u))
                (dh[i], self.hiddens[k+1][i], h_prev_l, h_prev_t))
                dh[i] = new_dh
                if i != 0:
                    dh[i-1] += dx
