...
class MLP0:
    ...
    def forward(self, A0):
        ...
        Z0 = np.dot(A0,self.layers[0].W.T) + np.dot(np.ones((A0.shape[0],1), dtype="f"),self.layers[0].b.T) # TODO
    ...
    def backward(self, dLdA1):
        ...
        if self.debug:
            ...
            self.dLdA0  = dLdA0
            self.dLdW0 = dLdW0
            #print(np.dot(dLdZ0.T,np.ones((self.A0.shape[0],1), dtype="f")))
            self.dLdb0 = dLdb0
...
class MLP1:
    ...
    def forward(self, A0):
        Z0 = np.dot(A0,(self.layers[0].W.T))+np.dot(np.ones((A0.shape[0],1), dtype="f"), self.layers[0].b.T) # TODO
        A1 = self.f[0].forward(Z0) # TODO
        x = self.layers[0].forward(A0) # TODO
        Z1 = np.dot(A1,self.layers[1].W.T)+np.dot(np.ones((A0.shape[0],1), dtype="f"), self.layers[1].b.T)  # TODO
...
class MLP4:
    ...
    def forward(self, A):
        ...
        for i in range(L):
            ...
            Z = np.dot(self.A[i],(self.layers[i].W.T))+np.dot(np.ones((self.A[i].shape[0],1), dtype="f"), self.layers[i].b.T)  # TODO
    ...
    def backward(self, dLdA):
        ...
        for i in reversed(range(L)):
            ...
            u = self.layers[i].backward(dLdZ) # TODO
